import MaterialIcons from "@expo/vector-icons/MaterialIcons";

export const InfoIcon = (props) => <MaterialIcons name="info" {...props} />;

export const HomeIcon = (props) => <MaterialIcons name="home" {...props} />;

export const SignInIcon = (props) => (
  <MaterialIcons name="account-box" size={24} color="black" {...props} />
);

export const SettingsIcon = (props) => (
  <MaterialIcons name="settings" {...props} />
);

export const ProfileIcon = (props) => (
  <MaterialIcons name="manage-accounts" size={24} color="black" {...props} />
);

export const Visibility = (props) => (
  <MaterialIcons name="visibility" size={24} color="black" {...props} />
);

export const VisibilityOff = (props) => (
  <MaterialIcons name="visibility-off" size={24} color="black" {...props} />
);

export const MenuIcon = (props) => (
  <MaterialIcons name="menu" size={24} color="black" {...props} />
);

export const FingerIcon = (props) => (
  <MaterialIcons name="fingerprint" size={24} color="black" {...props} />
);

export const GroupsIcon = (props) => <MaterialIcons name="group" {...props} />;
